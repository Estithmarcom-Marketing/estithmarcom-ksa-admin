import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Button } from "@/components/ui/button";

const Settings = () => {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">معلومات الموقع</h2>
      </div>

      <form className="space-y-6" dir="rtl">
        <div className="grid grid-cols-2 gap-x-4 gap-y-6">
          <div>
            <Label htmlFor="name-ar">اسم الموقع (عربي)</Label>
            <Input id="name-ar" placeholder="مثال: ميثاق" />
          </div>

          <div>
            <Label htmlFor="name-en">اسم الموقع (انجليزي)</Label>
            <Input id="name-en" placeholder="Ex: Mithaq" dir="ltr" />
          </div>

          <div>
            <Label htmlFor="phone">رقم الهاتف</Label>
            <Input id="phone" type="tel" placeholder="+966 XXX XXX XXX" dir="ltr" />
          </div>

          <div>
            <Label htmlFor="email">البريد الإلكتروني</Label>
            <Input id="email" type="email" placeholder="example@mithaq.com" dir="ltr" />
          </div>

          <div>
            <Label htmlFor="address">العنوان</Label>
            <Input id="address" placeholder="أدخل العنوان الكامل" />
          </div>
        </div>

        <Button type="submit">
          حفظ التغييرات
        </Button>
      </form>
    </div>
  );
};

export default Settings;